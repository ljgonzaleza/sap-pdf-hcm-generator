...
**Sigla de proyecto:** insertar `CONTPE` entre módulo y tipo/correlativo cuando aplique.  
**Ejemplo CONTPE:** `ZHHRCONTPE_770` (programa launcher), `ZHHRDCONTPE_001` (report batch).
...
**Sigla de proyecto CONTPE:** `ZHHRTCONTPE_<NNN>` → Ejemplo: `ZHHRTCONTPE_770`

### Grupos de funciones

Formato: `Z<Frente><Modulo>_<NNN>` → Ejemplo: `ZHHR_001`

**Sigla CONTPE:** `ZHHRCONTPE_<NNN>` → Ejemplo: `ZHHRCONTPE_001`

### Funciones

Formato: `Z<Frente><Modulo>_<TEXTO_DESCRIPTIVO>` → Ejemplo: `ZHHR_CLONE_EMPLOYEE`

**Sigla CONTPE:** `ZHHRCONTPE_<TEXTO>` → Ejemplo: `ZHHRCONTPE_GENERATE_PDF`
...
| Tabla CONTPE | `ZHR_CONTPE_<Sufijo>` | `ZHR_CONTPE_TEMPLATES`    |
| Campo        | `<Ambito>_<Sufijo>`  | `FCH_INGRESO`, `NMR_PERNR` |
| Data element | `ZDE_<nombre>`       | `ZDE_CLONE_ID`             |
| Data element CONTPE | `ZDE_CONTPE_<nombre>` | `ZDE_CONTPE_TPL_ID`  |
| Dominio      | `ZDO_<nombre>`       | `ZDO_CLN_STATUS`           |
| Dominio CONTPE | `ZDO_CONTPE_<nombre>` | `ZDO_CONTPE_STATUS`     |
| Índice       | `Z00`–`Z99`          | `Z01`                      |
| Paquete      | `Z<Modulo>[_<NNN>]`  | `ZHR` o `ZHR_001`          |
| Paquete CONTPE | `ZHR_CONTPE`       | Proyecto generador PDF     |
...
| CDS CONTPE | `ZHR_CONTPE_CDS_<NNNN>` |


---

## Mapeo nomenclatura — Proyecto CONTPE (Generador PDF HCM)

**Sigla proyecto**: `CONTPE`  
**Paquete**: `ZHR_CONTPE`  
**Referencia completa**: [docs/nomenclature-contpe.md](docs/nomenclature-contpe.md)

Patrón LATAM (análogo a `ZHR_CLN_CONFIG`): **`ZHR_CONTPE_<SUFIJO>`**

| Objeto LATAM | Código asignado | Función |
|--------------|-----------------|---------|
| Paquete | `ZHR_CONTPE` | Contenedor único del proyecto |
| Transacción | `ZHHRTCONTPE_770` | Mantenimiento plantillas PDF |
| Transacción | `ZHHRTCONTPE_771` | Generación documento PDF |
| Transacción | `ZHHRTCONTPE_772` | Consulta PDFs generados |
| Programa (tipo C) | `ZHHRCONTPE_770` | Launcher Fiori → plantillas |
| Programa (tipo C) | `ZHHRCONTPE_771` | Launcher Fiori → generación |
| Programa (tipo C) | `ZHHRCONTPE_772` | Launcher Fiori → histórico |
| Tabla | `ZHR_CONTPE_*` | Datos de aplicación |
| Elemento dato | `ZDE_CONTPE_*` | Tipos DDIC |
| Dominio | `ZDO_CONTPE_*` | Dominios DDIC |
| Clase | `ZCL_HHR_CONTPE_*` | Lógica OO |
| Interface | `ZIF_HHR_CONTPE_*` | Contratos |
| Excepción | `ZCX_HHR_CONTPE_*` | Manejo errores |
| Clase de mensajes | `ZHR_CONTPE` | Textos ES/EN/PT |
| CDS | `ZHR_CONTPE_I_*` / `ZHR_CONTPE_C_*` | Vistas RAP |
| OData | `ZUI_HHR_CONTPE_*_O4` | Servicios Fiori |
| App Fiori | `zhr_contpe_*` | Frontend SAPUI5 |
| Rol PFCG | `Z_HR_CONTPE_*` | Autorizaciones |

**Reglas aplicadas en código fuente (`src/`):**

- `AUTHORITY-CHECK` de `S_TCODE` al inicio de cada programa launcher; `LEAVE PROGRAM` si falla.
- Cabecera de programa en español con empresa LATAM.
- Sin textos hardcode en lógica (clase de mensajes `ZHR_CONTPE`).
- Parámetros método: `pi_`, `pe_`, `pt_`, `ps_`; objetos: `go_`, `lo_`.
- Un comando por línea; palabras clave ABAP en mayúsculas.
